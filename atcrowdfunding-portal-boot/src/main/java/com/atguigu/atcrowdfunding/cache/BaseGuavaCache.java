...
/**
 * 利用guava实现的内存缓存。缓存加载之后永不过期，后台线程定时刷新缓存值。刷新失败时将继续返回旧缓存。 在调用getValue之前，需要设置
 * refreshDuration， refreshTimeunit， maxSize 三个参数后台刷新线程池为该系 统中所有子类共享，大小为20.
 * 
 * @author **
 * @date 2018/6/8 15:07
 */
public abstract class BaseGuavaCache<K, V> {
	...
	/**
	 * 缓存最大容量 备注：maxiSize定义了缓存的容量大小，当缓存数量即将到达容量上线时，则会进行缓存回收，
	 * 回收最近没有使用或总体上很少使用的缓存项。需要注意的是在接近这个容量上限时就 会发生，所以在定义这个值的时候需要视情况适量地增大一点。
	 */
	protected int maxSize = 4;
	/**
	 * 缓存刷新线程池
	 */
	protected static ListeningExecutorService refreshThreadPool = MoreExecutors
	...
	/**
	 * 定义缓存值过期时的计算方法：一般是缓存过期时，重新读取数据库，缓存数据库中的数值（具体视情况而变） 新值计算失败时抛出异常，get操作将继续返回旧的缓存
	 * 
	 * @param key 缓存的key
	 * @return 缓存值
	 * @throws Exception 异常
				...
				if (cache == null) {
					...
					cache = cacheBuilder.build(new CacheLoader<K, V>() {
						...
						public V load(K k) throws Exception {
							// 为null时，会抛出异常
							System.out.println("==BaseGuavaCache===load=====key值="+k);
						...
						public ListenableFuture<V> reload(final K key, V oldValue) throws Exception {
							return refreshThreadPool.submit(new Callable<V>() {
								...
								public V call() throws Exception {
									System.out.println("==BaseGuavaCache===call======");
	 ...
	 * @param key 键
	 * @return 值
	 * @throws Exception 异常 备注：该key对应的值在缓存中不存在或者过期，调用该方法就会抛出异常。这个方法必须显式抛出异常，
	 *                   以供业务层判断缓存是否存在以及是否过期
	 */
	public V getValue(K key) throws Exception {
		try {
			...
			LOGGER.error("getValue从内存缓存中获取内容时发生异常，key:" + key, e);
			throw e;
		}
	}
	...
	public V getValueOfDefault(K key, V defaultValue) {
		try {
			...
			LOGGER.error("getValueOfDefault从内存缓存中获取内容时发生异常，key:" + key, e);
			return defaultValue;
		}
	}
	 ...
	 * 设置缓存过期周期(链式编程)
	 */
	public BaseGuavaCache<K, V> setExpirationDuration(int expirationDuration) {
	 ...
	 * 设置缓存过期周期时间单元(链式编程)
	 */
	public BaseGuavaCache<K, V> setExpirationTimeUnit(TimeUnit expirationTimeUnit) {
