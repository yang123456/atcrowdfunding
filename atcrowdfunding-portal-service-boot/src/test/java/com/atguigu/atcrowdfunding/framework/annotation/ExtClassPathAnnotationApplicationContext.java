package com.atguigu.atcrowdfunding.framework.annotation;
/**
...
import com.atguigu.atcrowdfunding.framework.ClassUtil;
...
public class ExtClassPathAnnotationApplicationContext {
    ...
    public ExtClassPathAnnotationApplicationContext(String path){
        this.path = path;
    }
    ...
    private void initAttribute(Object object)throws Exception{
        ...
        for(Field field : fields){
            //查找字段上有依赖的注解
            boolean falg = field.isAnnotationPresent(ExtResource.class);
            if (falg){
            	ExtResource iocResource = field.getAnnotation(ExtResource.class);
                if (iocResource!=null){
                    //获取属性的beanid
    ...
    private List<Class>findAnnoationService()throws Exception{
        ...
        if (classes==null || classes.size()==0){
            throw new Exception("not found service is added annoation for @ExtService");
        }
        List<Class> annoationClasses = new ArrayList<Class>();
        for (Class clazz:classes){
            //通过反射机制 查找增加了注解的类
        	ExtService iocService = (ExtService) clazz.getDeclaredAnnotation(ExtService.class);
            if (iocService!=null){
