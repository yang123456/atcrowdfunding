...
public class MyThreadPoolExecutor implements Executor {
	...
	public MyThreadPoolExecutor(String name, int coreSize, int maxSize, BlockingQueue<Runnable> taskQueue,
	...
	private boolean addWorker(Runnable newTask, boolean core) {
		...
		for (;;) {
			...
			if (runningCount.compareAndSet(count, count + 1)) {
				// 线程的名字
				String threadName = (core ? "core_" : "") + name + runningCount.incrementAndGet();
				// 创建线程并启动
				new Thread(() -> {
