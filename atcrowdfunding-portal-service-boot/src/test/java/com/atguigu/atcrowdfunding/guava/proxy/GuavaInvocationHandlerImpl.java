...
public class GuavaInvocationHandlerImpl<T> implements InvocationHandler {
    ...
    public GuavaInvocationHandlerImpl(Class<T> targetInterface, T targetObject) {
    	...
    	Preconditions.checkArgument(targetInterface.isInterface(), "%s 不是一个接口类", targetInterface);
        ...
        Preconditions.checkArgument(valid, "%s 必须实现 %s", targetObject.getClass().getName(), targetInterface.getName());
